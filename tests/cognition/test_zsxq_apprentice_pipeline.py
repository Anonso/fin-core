...
def test_deep_read_golden_style_article(tmp_path: Path):
    """规则缝 verbatim 提取驱动全链结果构造（units→clusters→clocks→suggestions）。
...
def test_deep_read_fengxianjun_teacher_cognition(tmp_path: Path):
...
def test_external_context_article_yields_no_units(tmp_path: Path):
