...
## 已知边界 → 09-05 owner 裁决后当日落地（parser v3）

- 9/5 起评分表出现「项目评分/情绪热度」新列与分号单行 k:v 格式：owner
  当日裁决「新旧格式并存支持、项目评分=利好度改版、情绪热度÷10=共识度」，
  已随 parser v3 落地（别名扩展 + `_iter_key_values` 分号拆分；「投产启动」
  同义启动时机；「供货周期」不映射、防落 horizon「周期」兜底）。
- 名称-代码冲突：owner 裁决「正文显式代码优先（利通电子（603629）为准）」，
  `normalize_draft_code` 改为只在缺码时补名册；v2 误码时代的
  `_is_code_name_mismatch` 陈旧行清除随之删除（mismatch 是新语义下的合法
  行）。表载体的文字级误码纠正（`normalize_inline_codes`）保留不动——
  密集表格单元格的 vision 误码防护仍是 D-037 语义。920225 陈旧行已 drop。
- 随手修：`manage_instrument_scores.py` 的「drop <id>」argparse 顺位 bug
  （record_id 挂子解析器；文档用法此前从未可用）。
