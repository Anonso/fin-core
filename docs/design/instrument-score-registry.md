...
- 门槛（owner 2026-09-02 晚修订）：普通栏与 Q&A 有评分且能量评分 <7 的
  ...
  漏爬可审计。
  → 2026-09-03 被 D-037 修订：增量门槛改能量 ≥6.0（<6.0 跳过）；
  评分表按“发布时间倒序时间线”查询/解读，最新=锚、旧行=趋势；
  解析器补代码前置 inline 格式，回填口径对齐 ≥6.0。详见
  [instrument-score-timeline.md](instrument-score-timeline.md)。
