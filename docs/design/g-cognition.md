...
| 长认知 read-model | `CognitionMainlinePublisher`（构建期唯一 writer）/ `CognitionMainlineReadModelReader`（只读） | Git 外 `$XDG_STATE_HOME/fin-analyse/cognition-mainline-readmodel-v1/`；schema `fin.cognition-mainline-readmodel/v1`（`cognition_mainline_readmodel.py:30`） | 增量生长制（g-mainline-growth-v1，2026-09-04 上线：机器提名→起草 span 机验→owner 扫批逐段归属；设计稿规则5 归档见 git 史 3332912）；rebuild 双触发=ZSXQ ingest + 标注指纹独立触发（缺口 B，e5bcddf）；build 期仍从标注文档确定性生成，现 generation 54 / evolution 13 节点（2026-09-04 17:15）；validator 闭集（7 值 cognition_mode、5 值 relation）任一失败整份拒绝；numeric-generation CAS（:794，disposition PUBLISHED/ALREADY_PUBLISHED/REJECTED） |
