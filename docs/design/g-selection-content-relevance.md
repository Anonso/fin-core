...
- “长电科技 封测”：llm_context 含 8/13 特刊（原先“科技”滑窗碎片灌高
  “科技主线”泛特刊，8/13 落 rank 5/10；碎片入停用词表后闭环，BUG-023）。

回归：guo 大集 547 passed（新增装配竞争/冻结/端到端 + BUG-023 碎片 5 测）；
全仓 3025 passed。剩余边界：公司名不从问句解析（方案 C 既知边界）；若其他
公司名高频后缀碎片（电子/材料等）再现同类误排，再考虑公司名解析进
companies，不预建。
