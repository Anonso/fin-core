...
| **外部审视** | CC 专属统一外部评审机制：CC 的评审者固定 scripts/codex_open.sh --sandbox read-only（当前 codex-glm·glm-5.3·max，换规格只改脚本）；三触发各一次=设计门/吓人 diff/外援；packet 冻结四问；评审只产发现、裁决归 CC；裁决记录附时长。codex-open 不设设计门/外部审计，全部自己完成。 | [AGENTS.md](../AGENTS.md) 审查机制归属 |
