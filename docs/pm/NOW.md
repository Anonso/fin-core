...
| 主线 | 3 | 标的评分维护列表 + ZSXQ 窗口分级：已交付（回填 581 条〔index 内全部可解析 ≥6：60 天 + 历史 6/24 起〕、read_instrument_scores 时间线 + read_article_search 双查已接 thin server、G/reference 窗口分级落地；增量门槛 6.0 走 config/zsxq_capture.json〔D-036/037〕）；09-03 实弹 sync 过（8/29 长电 8.6/8.8 锚 + 7/7 参照）；5/13–6/23 旧 OCR/图片表（179 篇含图、24 篇带 OCR 文本）未结构化待评估；首篇 [6,7) 或 <6 新帖边界样本待自然窗口 | 排期见 [../design/instrument-score-registry.md](../design/instrument-score-registry.md) |
