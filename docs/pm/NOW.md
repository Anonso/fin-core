...
| 旁路·时间（09-07 23:00 后） | 33 | 回放证据层定时首验：核 daily.log 9/07 批次实跑非 skip（batch=20260907，nominations-20260907 落盘）；节假日空转 9/06 已实测 rc=0；取数全败=rc1 硬错误（代理抖动守卫 09-06 实测） | 09-07 23:00 后 |
| 旁路·时间（09-30） | 31 | 回放证据层方向类窗口到期：CHK-0827-01/02-spread、CHK-0827-02-blowup 暂钉 9/30 到期后跑 nominate 出 relation（改判随时可做=retire+新 spec）；扩篮子已拍板不扩（09-06，中证银行单代码，mapping 留痕）；blowup 检查机器面 not_machine_v1，到期 relation=unknown 需 owner 供证据或接受 unknown | 09-30 后首个验证批次 |
