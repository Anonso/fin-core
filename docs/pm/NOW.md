...
| 旁路·使用 | 36 | 退役墓碑闸已上线（设计门 cmd P1×1+P2×4 全采纳）：config/zsxq_retired.json registry + consumer 收尾清扫挂点（manifest 三条件核身才删，永不阻断 ingest）+ 退役 skill 登记步骤；真 KB 实弹 CLEAN | 随下次真实退役动作复验 |
| 旁路·使用 | 34 | 元认知调节器已生效：stdio 按会话拉起（无常驻单元），新问询即带块；画像 v2 在 state、墙钟修复 448f862、A/B 单腿对比进行中（台账 meta-calibrator-ab-20260907，6/20 对） | A/B 全量报告已出（09-08）：20 对全成、零泄漏零反向、基线强+调节器边缘加固（背书滑移 1 处命中、q05 来源层级分歧待复跑归因）；台账 meta-calibrator-ab-20260907/report.md；q05 异常已归因=退役材料复活污染（复跑 6/6 零降级，#36），与调节器无关 |
| 旁路·时间 | 33 | 回放线定时首验收敛（09-08）：机制 rc=0/守卫/空转全过；9/7 交易日 23:00 skip 实证=新浪当日行发布延迟（09:10 复核已出，close 3932.699）→ wrapper 加交易日 35 分钟×4 重试（次日批全窗重取兜底，链无损）；设计稿已删（家规5） | 今晚 23:00 重试版首验 |
