...
| 位置 | 序 | 事项 | 等谁 / 何时 |
| --- | --- | --- | --- |
| 主线 | 2 | BUG-012 定修：描述+供料两刀已施工（08-30，全量 2901 绿；当晚复测 2902 绿，用例数随施工漂移、非固定基线）；待实弹探针闭环（当日普通栏提问 → 工具被调 items 非空；公告类 → 走 external_evidence） | 探针随下次问询（owner 08-30 晚：空坍塌定修在修，合入后再实弹，勿提前） |
