...
_TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "read_instrument_scores": (
        ...
        "LOCAL COVERAGE BOUNDARY (D-037): only rating tables from articles with "
        "energy score >= 6.0 are indexed; energy <6 rows are NOT in this registry "
        "even though full-text search still sees the articles. Records return "
        "newest first by publish time: the newest row is the current anchor and "
        "older rows are history/trend. An empty or old result does NOT mean ZSXQ "
        "has no newer table — pair this read with read_article_search to bound "
        "what the local library covers before claiming 'no update'. "
